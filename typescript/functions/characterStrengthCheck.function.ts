...
import characterStrengthCheckConstraints from "../data/interfaces/characterStrengthCheckConstraints.interface";
 ...
 */
export default function characterStrengthCheck(
    password:string,
    constraints:characterStrengthCheckConstraints= {
        min_passwordLength:3,
        max_passwordLength:256
    }
):number{

    const uniqueCharacters:Array<string> = [];
    ...
    return (
        (
            (
                (
                    (
                        uniqueCharactersAverage
                        + repeatingCharacterAverage
                    )/2
                )
                + min_passwordLengthAverage
                + max_passwordLengthAverage
            )/3
        )*100
    );
